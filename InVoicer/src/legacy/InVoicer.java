...
public class InVoicer {
	...
	/*
	 * TODO: option to select preexisting clients idea: add a little arrow on the
	 * right side of each cell that when clicked creates a jcombobox thing
	 * ------------- | | | -- | | |V | | | -- | ------------- CURRENT ISSUE IN
	 * ARROWCELLRENDERER: NOT PROPERLY REGISTERING CLICK FIX THIS optional: notes
	 * section? just add another column or smthn
	 */
...
*/
			...
			public void windowClosing(java.awt.event.WindowEvent windowEvent) {
				if (saveStatus)
					...
					switch (JOptionPane.showConfirmDialog(null,
							"Some changes may not have been saved. Save before closing?")) {
					case 0:
						saveTable();
						System.exit(0);
						break;
	...
	void loadListFromFile() {
		try {
			...
			/*
			 * Invoice in = new Invoice(600,"John Cliente"); Invoice j = new
			 * Invoice(4567.88,"Sammy Rich"); invoiceList.add(in); invoiceList.add(j);
			 */
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	void saveTable() {
		...
		try {
			bw = new BufferedWriter(new FileWriter(
					new File(FileSystemView.getFileSystemView().getDefaultDirectory().getPath() + "/invoiceData.csv")));
			bw.write("");
			for (int i = 0; i < jt.getRowCount(); i++) {
				String s = "";
				for (int j = 0; j < 5; j++) {
